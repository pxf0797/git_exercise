...
class cpnShell:
    ...
    def cpnHeaderGenerateVfbMcpnComment(self,name):
        ...
        comments += ('\n'*1)
        comments += ('    // 组件管理输出功能函数\n')
        comments += ('    //void (*output)(hvfbM%s t);\n    //TODO\n' %name)
        comments += ('\n'*1)
        comments += ('    // 注入组件输出类及其他功能类\n    INJ(vfbO%s, vfbO%s);\n' %(name,name))
    ...
    def cpnSourceGenerateVfbMcpnComment(self,name):
        ...
        comments += self.cpnHeaerAndSourceClassFuncComment('vfbM'+name,'output',funcEntity,'组件管理输出功能函数')
        funcEntity = ('(vfbM%s){\n' %name)
        funcEntity += ('    // 功能函数配置\n')
        funcEntity += ('    cthis->init = vfbM%s_init;\n' %name)
        funcEntity += ('    //cthis->output = vfbM%s_output;\n' %name)
        funcEntity += ('    //TODO\n')
        funcEntity += ('\n    return cthis;\n}')
